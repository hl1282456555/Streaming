...
DEFINE_LOG_CATEGORY(LogRTMP);

#define LOCTEXT_NAMESPACE "FRTMPModule"

void FRTMPModule::StartupModule()
{
	bInitialized = false;
	...
	UE_LOG(LogRTMP, Log, TEXT("FFmpeg AVCodec version: %d.%d.%d"), LIBAVFORMAT_VERSION_MAJOR, LIBAVFORMAT_VERSION_MINOR, LIBAVFORMAT_VERSION_MICRO);
	UE_LOG(LogRTMP, Log, TEXT("FFmpeg license: %s"), UTF8_TO_TCHAR(avformat_license()));
}

void FRTMPModule::ShutdownModule()
{
...
void FRTMPModule::FFmpegCallback(void*, int Level, const char* Format, va_list ArgList)
{
	char buffer[2048]{ 0 };
	...
	{
	case AV_LOG_TRACE:
		UE_LOG(LogRTMP, VeryVerbose, TEXT("%s"), *logStr);
		break;
	case AV_LOG_DEBUG:
		UE_LOG(LogRTMP, Verbose, TEXT("%s"), *logStr);
		break;
	case AV_LOG_VERBOSE:
		UE_LOG(LogRTMP, Verbose, TEXT("%s"), *logStr);
		break;
	case AV_LOG_INFO:
		UE_LOG(LogRTMP, Log, TEXT("%s"), *logStr);
		break;
	case AV_LOG_WARNING:
		UE_LOG(LogRTMP, Warning, TEXT("%s"), *logStr);
		break;
	case AV_LOG_ERROR:
		UE_LOG(LogRTMP, Error, TEXT("%s"), *logStr);
		break;
	case AV_LOG_FATAL:
		UE_LOG(LogRTMP, Fatal, TEXT("%s"), *logStr);
		break;
	default:
		UE_LOG(LogRTMP, Log, TEXT("%s"), *logStr);
		break;
...
void FRTMPModule::InitLibraryHandles()
{
	if (bInitialized) {
		return;
	}
	...
	if (AVCodecHandle == nullptr || AVDeviceHandle == nullptr || AVFilterHandle == nullptr ||
		...
		UE_LOG(LogRTMP, Error, TEXT("Load dependecy dll failed."));
		return;
	}
...
void FRTMPModule::UnloadHandledLibraries()
{
	bInitialized = false;
...
void* FRTMPModule::LoadDependencyLibrary(const FString& DLLName)
{
	FString baseDir = IPluginManager::Get().FindPlugin("RTMP")->GetBaseDir();
	...
	UE_LOG(LogRTMP, Log, TEXT("Loading dependency dll ----> %s"), *dllFilename);
...
IMPLEMENT_MODULE(FRTMPModule, RTMP)